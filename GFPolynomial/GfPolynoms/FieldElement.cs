...
namespace GFPolynoms
{
    ...
    public class FieldElement
    {
        ...
        private void ValidateArgument(FieldElement b)
        {
            ...
            {
                string res = "Field {0}({1}) is not compatible to field {2}({3})";
                //$"Field {Field}({Field.GetType()}) is not compatible to field {b.Field}({b.Field.GetType()})"
                throw new ArgumentException(string.Format(res,Field,Field.GetType(),b.Field,b.Field.GetType()));
            }
        }
