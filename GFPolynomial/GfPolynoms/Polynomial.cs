    ...
    public class Polynomial
    {
        ...
        public Polynomial(GaloisField field, params int[] coefficients)
        {
            ...
            if (coefficients == null)
                throw new ArgumentNullException(NameOfExtension.nameof(() => coefficients));

            Field = field;

            _coefficients = coefficients.ToList();
            if (_coefficients.Any(x => Field.IsFieldElement(x) == false))
                throw new ArgumentException(NameOfExtension.nameof(() => coefficients));
        ...
        public bool IsZero
        {
            get
            {
                return Degree == 0 && _coefficients[0] == 0;
            }
        }
